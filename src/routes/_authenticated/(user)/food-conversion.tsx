...
import {
  calculateNutrition,
  getSelectedFood,
} from '@/modules/food-conversion/helpers'
...
function FoodCalculator() {
  ...
  const [selectedSourceFood, setSelectedSourceFood] = useState<
    GetAllFoods | undefined
  >(undefined)
  const [selectedTargetFood, setSelectedTargetFood] = useState<
    GetAllFoods | undefined
  >(undefined)
  ...
  const form = useFormConversion({
    ...
    validate: {
      ...
      amount: (value) =>
        !value || value <= 0 ? 'Please enter a valid amount' : null,
    },
  ...
  const handleSubmit = form.onSubmit((values) => {
    ...
    const targetAmount =
      (Number(values.amount) * caloriesPerUnit.source) / caloriesPerUnit.target
...
export const Route = createFileRoute('/_authenticated/(user)/food-conversion')({
